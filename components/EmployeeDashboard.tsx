    ...
    return (
      <div className="h-full flex flex-col animate-in fade-in duration-500">
        <div className="bg-slate-900 p-3 flex justify-between items-center border-b border-white/10 shrink-0">
          ...
          <button onClick={() => setViewMode('dashboard')} className="px-4 py-1.5 bg-white text-slate-900 font-black text-[10px] uppercase hover:bg-slate-100 transition-all rounded-none shadow-md">الرجوع</button>
        </div>
    ...
    <div className="space-y-8 pb-16 bg-slate-50/50 min-h-screen" dir="rtl">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-6 w-full md:w-auto">
            <div className="relative group">
              <div className="w-16 h-16 bg-slate-900 rounded-none flex items-center justify-center text-3xl shadow-lg border border-slate-800 transition-transform group-hover:rotate-6">👤</div>
              <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-teal-500 border-4 border-white rounded-none"></div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-10">
              ...
              <div className="space-y-1 text-right">
                ...
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-none text-xs font-black tabular-nums border border-slate-200">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end border-t md:border-t-0 pt-4 md:pt-0 md:pr-6 md:border-r border-slate-100">
             <button onClick={() => setIsAIPanelOpen?.(true)} className="flex-1 md:flex-none px-6 py-2.5 bg-slate-900 text-white rounded-none text-[10px] font-black uppercase hover:bg-teal-600 transition-all shadow-md flex items-center justify-center gap-2 group">
               <span className="group-hover:animate-pulse">🎤</span> مساعد ذكي
             </button>
             <button onClick={handleExit} className="px-6 py-2.5 bg-rose-50 text-rose-600 rounded-none text-[10px] font-black uppercase border border-rose-100 hover:bg-rose-600 hover:text-white transition-all">
               خروج 🚪
             </button>
          </div>
        </div>
      ...
      <div className="px-4 md:px-12 space-y-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div onClick={() => handleOpenModal('إجمالي محفظتك', myPortfolio)} className="bg-slate-900 p-6 rounded-none text-white shadow-premium cursor-pointer hover:bg-slate-800 transition-all group overflow-hidden relative">
          ...
          <div onClick={() => handleOpenModal('سدادك الفعلي', myPortfolio.filter(c => (c.paymentAmount || 0) > 0))} className="bg-teal-600 p-6 rounded-none text-white shadow-premium cursor-pointer hover:bg-teal-700 transition-all relative overflow-hidden">
          ...
          <div className="bg-white p-6 rounded-none border border-slate-200 shadow-soft">
          ...
          <div onClick={() => handleOpenModal('إجمالي الحسابات', myPortfolio)} className="bg-white p-6 rounded-none border border-slate-200 shadow-soft cursor-pointer hover:bg-slate-50 transition-all">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div onClick={() => handleOpenModal('تمويل شخصي (BF)', stats.globalProducts.bf)} className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-indigo-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">🏦</div>
            ...
            <div onClick={() => handleOpenModal('تمويل تأجيري (AL)', stats.globalProducts.al)} className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-teal-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">🚗</div>
                <div className="text-right">
            ...
            <div onClick={() => handleOpenModal('بطاقات ائتمان (CC)', stats.globalProducts.cc)} className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-amber-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">💳</div>
                <div className="text-right">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            <div 
              onClick={() => handleOpenModal('محفظة المتوفين', stats.deceased.bf.concat(stats.deceased.al).concat(stats.deceased.cc))}
              className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-slate-100 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">💀</div>
                <div className="text-right">
                  <h3 className="text-xs font-black text-slate-800 uppercase">محفظة المتوفين</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Deceased Cases</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">عدد الحسابات</p>
                  <p className="text-xl md:text-2xl font-black text-slate-800 tabular-nums">{stats.deceased.totalCount.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">إجمالي المبلغ</p>
                  <p className="text-xl md:text-2xl font-black text-slate-900 tabular-nums">SAR {stats.deceased.totalAmount.toLocaleString()}</p>
                </div>
              </div>
            </div>

            <div 
              onClick={() => handleOpenModal('محفظة الرواتب', stats.salary.bf.concat(stats.salary.al).concat(stats.salary.cc))}
              className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-indigo-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">💰</div>
                <div className="text-right">
                  <h3 className="text-xs font-black text-slate-800 uppercase">محفظة الرواتب</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Salary Transfer</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">عدد الحسابات</p>
                  <p className="text-xl md:text-2xl font-black text-slate-800 tabular-nums">{stats.salary.totalCount.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">إجمالي المبلغ</p>
                  <p className="text-xl md:text-2xl font-black text-indigo-600 tabular-nums">SAR {stats.salary.totalAmount.toLocaleString()}</p>
                </div>
              </div>
            </div>

            <div 
              onClick={() => handleOpenModal('حديثي التعثر (0-1)', stats.recent01.bf.concat(stats.recent01.al).concat(stats.recent01.cc))}
              className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-teal-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">⏳</div>
                <div className="text-right">
                  <h3 className="text-xs font-black text-slate-800 uppercase">حديثي التعثر (0-1)</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">New Delinquent</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">عدد الحسابات</p>
                  <p className="text-xl md:text-2xl font-black text-slate-800 tabular-nums">{stats.recent01.totalCount.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">إجمالي المبلغ</p>
                  <p className="text-xl md:text-2xl font-black text-teal-600 tabular-nums">SAR {stats.recent01.totalAmount.toLocaleString()}</p>
                </div>
              </div>
            </div>

            <div 
              onClick={() => handleOpenModal('حديثي التعثر (1-2)', stats.recent12.bf.concat(stats.recent12.al).concat(stats.recent12.cc))}
              className="bg-white p-6 rounded-none border border-slate-200 shadow-soft hover:shadow-md transition-all cursor-pointer group"
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-amber-50 rounded-none flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">⚠️</div>
                <div className="text-right">
                  <h3 className="text-xs font-black text-slate-800 uppercase">حديثي التعثر (1-2)</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Late Stage New</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">عدد الحسابات</p>
                  <p className="text-xl md:text-2xl font-black text-slate-800 tabular-nums">{stats.recent12.totalCount.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-1">إجمالي المبلغ</p>
                  <p className="text-xl md:text-2xl font-black text-amber-600 tabular-nums">SAR {stats.recent12.totalAmount.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4 text-right">
            ...
            <div className="bg-white rounded-none border border-slate-200 shadow-soft overflow-hidden h-[320px] flex flex-col w-full">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-50">
                {stats.topAvailableBalances.map((c, i) => (
                  <div key={c.id} onClick={() => setSelectedCustomer(c)} className="p-3.5 grid grid-cols-3 items-center text-[10px] hover:bg-slate-50 cursor-pointer transition-colors group">
                    <div className="text-right font-black text-slate-700 truncate flex items-center gap-2 min-w-0">
                      <span className="w-4 h-4 rounded-none bg-emerald-50 text-emerald-600 flex items-center justify-center text-[7px] shrink-0 font-black border border-emerald-100">{i+1}</span>
            ...
            <div className="bg-white rounded-none border border-slate-200 shadow-soft overflow-hidden h-[320px] flex flex-col w-full">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-50">
                {stats.highBalance.map((c, i) => (
                  <div key={c.id} onClick={() => setSelectedCustomer(c)} className="p-3.5 grid grid-cols-3 items-center text-[10px] hover:bg-slate-50 cursor-pointer transition-colors group">
                    <div className="text-right font-black text-slate-700 truncate flex items-center gap-2 min-w-0">
                      <span className="w-4 h-4 rounded-none bg-slate-100 text-slate-500 flex items-center justify-center text-[7px] shrink-0 font-black border border-slate-200">{i+1}</span>
        ...
        <section className="space-y-4 text-right">
          ...
          <div className="bg-white rounded-none border border-slate-200 shadow-soft overflow-hidden h-[350px] flex flex-col w-full">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-50">
              {stats.sabilData.length > 0 ? stats.sabilData.map((c, i) => (
                <div key={c.id} onClick={() => setSelectedCustomer(c)} className="p-4 grid grid-cols-3 items-center text-[11px] hover:bg-indigo-50/30 cursor-pointer transition-colors group">
                  <div className="text-right font-black text-slate-700 truncate flex items-center gap-3 min-w-0">
                    <span className="w-5 h-5 rounded-none bg-slate-100 text-slate-500 flex items-center justify-center text-[8px] shrink-0 font-black border border-slate-200">{i+1}</span>
                  ...
                  <div className="text-left">
                    <span className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-none text-[9px] font-black tabular-nums border border-indigo-200">
                      {c.sabilOrderNumber}
                    </span>
                  </div>
                </div>
        ...
        <section>
          <div 
            ...
            className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-8 md:p-12 rounded-none shadow-premium flex flex-col md:flex-row items-center justify-between group cursor-pointer hover:scale-[1.01] transition-all relative overflow-hidden"
          >
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-teal-500/10 rounded-none blur-3xl group-hover:bg-teal-500/20 transition-all"></div>
            <div className="flex items-center gap-8 relative z-10 text-center md:text-right flex-col md:flex-row">
              <div className="w-24 h-24 bg-white/5 backdrop-blur-2xl rounded-none flex items-center justify-center text-5xl shadow-2xl border border-white/10 group-hover:rotate-6 transition-transform">🤖</div>
            ...
            <div className="mt-8 md:mt-0 px-10 py-4 bg-teal-600 text-white rounded-none font-black text-xs uppercase shadow-2xl group-hover:bg-teal-500 transition-all z-10 border border-teal-400/30">
              تحدث مع ذكي
            </div>
          </div>
        </section>
        ...
        <section>
          <div 
            ...
            className="bg-white p-8 rounded-none border border-slate-200 shadow-soft flex flex-col md:flex-row items-center justify-between group cursor-pointer hover:bg-slate-50 transition-all relative overflow-hidden"
          >
            <div className="flex items-center gap-6 relative z-10 text-center md:text-right flex-col md:flex-row">
              <div className="w-16 h-16 bg-slate-900 rounded-none flex items-center justify-center text-3xl shadow-xl border border-slate-800 group-hover:rotate-6 transition-transform">📂</div>
              <div className="text-right">
            ...
            <div className="mt-6 md:mt-0 px-8 py-3.5 bg-slate-100 text-slate-600 rounded-none font-black text-[10px] uppercase shadow-sm group-hover:bg-slate-900 group-hover:text-white transition-all z-10 border border-slate-200">افتح المحفظة</div>
          </div>
        </section>
      </div>
